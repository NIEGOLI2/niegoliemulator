(async function() {
    ...
    // Allow integrator selection of EmulatorJS version via window.EJS_selectedVersion (set by settings UI).
    // If provided, use official CDN layout: https://cdn.emulatorjs.org/<version>/data/
    let scriptPath;
    if (typeof window.EJS_selectedVersion === "string" && window.EJS_selectedVersion.trim()) {
        let ver = window.EJS_selectedVersion.trim();
        if (!ver.endsWith("/")) ver += "/";
        // CDN base - uses the EmulatorJS CDN distribution structure
        scriptPath = "https://cdn.emulatorjs.org/" + ver + "data/";
    } else {
        scriptPath = (typeof window.EJS_pathtodata === "string") ? window.EJS_pathtodata : folderPath((new URL(document.currentScript.src)).pathname);
        if (!scriptPath.endsWith("/")) scriptPath += "/";
    }
    //console.log("loader scriptPath:", scriptPath);
    ...
    // Ensure EJS_threads is always defined. If integrator intentionally set window.EJS_threads it will be preserved.
    // Default to enabling threads when SharedArrayBuffer is present (best-effort); integrator-provided value wins.
    window.EJS_threads = (typeof window.EJS_threads === "boolean")
        ? window.EJS_threads
    ...
    // Let EmulatorJS handle its own Virtual Gamepad configuration internally.
    ...
    // Ensure a safe numeric default for volume so EmulatorJS.setVolume doesn't attempt to access undefined audio internals.
    // Preserve any integrator-provided numeric setting.
    if (typeof window.EJS_volume !== "number") {
    ...
    // Construct EmulatorJS safely — if a core tries to use audio internals (Module.AL) too early
    // we catch the error, provision safe defaults, and retry once to avoid crashing the loader.
    try {
        window.EJS_emulator = new EmulatorJS(EJS_player, config);
    } catch (err) {
        // Detect errors where emulator tried to access audio internals (Module.AL) too early.
        // Provide conservative safe defaults and retry once.
        console.warn("EmulatorJS construction failed, provisioning safe fallbacks and retrying:", err && err.message ? err.message : err);
        ...
        // Ensure a minimal global Module exists and has an AL object to avoid undefined property access.
        // This stub uses no-op functions to satisfy early calls; the real core/runtime may replace these later.
        window.Module = window.Module || {};
        window.Module.AL = window.Module.AL || {
            // common AL-like no-op placeholders (some cores may call one of these)
            alSourcef: function () {},
            alSource3f: function () {},
            alListenerf: function () {},
            alGetError: function () { return 0; },
            // generic fallback property access
            __stub__: true
        };
        ...
        try {
            ...
            // If retry fails, rethrow so the integrator can see the error.
            console.error("EmulatorJS construction retry failed:", err2);
            throw err2;
        }
    }
